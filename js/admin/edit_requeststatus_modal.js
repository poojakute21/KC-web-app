$(document).ready(function(){
    ...
    $("#editForm").submit(function(event){
        ...
        $("#editForm input[name='requeststatusName']").val($("#editForm input[name='requeststatusName']").val().trim());
        var emptyFields = [];
        // check empty fields
        if ($("#editForm input[name='requeststatusName']").val().trim().length === 0) {
            emptyFields.push("Request Status Name");
        }

        var requeststatusStatus_typeData = [];
        $.each($("#editForm input[name='requeststatusStatus']:checked"), function(){
        ...
        $.ajax({
            ...
            type: 'POST',
            data:{'type' : 'updaterequeststatus' , 'uData' : formData},
